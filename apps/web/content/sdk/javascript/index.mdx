...
- `@canarygate/sdk/js/client` — for browsers. Always uses the snapshot request and never opens SSE.
- `@canarygate/sdk/js/server` — for server runtimes (Node.js, Deno, Bun, Edge). Supports real-time SSE streaming with `stream: true`.

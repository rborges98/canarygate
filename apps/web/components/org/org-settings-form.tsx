...
export function OrgSettingsForm({ orgId, initialName, initialSlug }: Props) {
  ...
  async function onSubmit(data: FormValues) {
    await updateOrg(orgId, { name: data.name, slug: data.slug })
    router.refresh()
  }
      ...
      >
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-4">
            <div className="border-cg-indigo-600 bg-cg-indigo-950 text-cg-indigo-100 flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-xl border text-[22px] font-bold">
              {name[0]?.toUpperCase()}
            </div>
          ...
          <div>
            ...
            <div className="border-cg-bg-100 flex overflow-hidden rounded-lg border">
              <span className="border-cg-bg-100 bg-cg-bg-200 text-cg-neutral-500 flex-shrink-0 border-r px-3 py-2.5 font-mono text-[12px]">
                canarygate.com/
              </span>
              <input
